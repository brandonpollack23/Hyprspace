...
#### Labels
Each workspace gets a label under it: `id: name`, or just the id when the workspace has no name of its own. The label's height comes out of the workspace boxes, so the panel keeps its size.
- `plugin:overview:showLabels` draw the labels (default: true)
- `plugin:overview:labelFontSize` in logical pixels (default: 14)
- `plugin:overview:labelFont` font family; empty uses `misc:font_family`
- `plugin:overview:labelMargin` gap between a workspace and its label (default: 6)
- `plugin:overview:labelColor`
- `plugin:overview:labelActiveColor` color of the active workspace's label, which is also drawn bold
