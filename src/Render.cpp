...
// Workspace label textures, keyed by everything that changes their pixels.
// Only created and dropped from draw(), while the renderer is current.
static std::unordered_map<std::string, SP<Render::ITexture>> labelCache;

void clearLabelCache() {
    labelCache.clear();
}

// "id: name", or just the id for a workspace without a name of its own.
static std::string workspaceLabel(int wsID, PHLWORKSPACE ws) {
    if (wsID == SPECIAL_WORKSPACE_START)
        return "special";
    const std::string id = std::to_string(wsID);
    if (!ws || ws->m_name.empty() || ws->m_name == id)
        return id;
    return id + ": " + ws->m_name;
}

static SP<Render::ITexture> labelTexture(const std::string& text, const CHyprColor& color, int fontSize, int maxWidth, int weight) {
    const auto key = std::format("{}\x1f{:x}\x1f{}\x1f{}\x1f{}", text, color.getAsHex(), fontSize, maxWidth, weight);
    if (const auto it = labelCache.find(key); it != labelCache.end())
        return it->second;

    // workspaces come and go, so don't let renamed or removed ones pile up
    if (labelCache.size() > 256)
        labelCache.clear();

    // an empty font falls back to misc:font_family; maxWidth ellipsizes long names
    auto tex = g_pHyprRenderer->renderText(text, color, std::max(fontSize, 1), false, config.labelFont->value(), maxWidth, weight);
    labelCache[key] = tex;
    return tex;
}

// Label under a workspace box; boxes and the clip are monitor-local pixels.
static void renderLabel(const CBox& wsBox, const std::string& text, const CHyprColor& color, int weight, double margin, int fontSize, const CBox& clip) {
    const auto tex = labelTexture(text, color, fontSize, (int)wsBox.w, weight);
    if (!tex || tex->m_size.x <= 0 || tex->m_size.y <= 0)
        return;

    CTexPassElement::SRenderData data;
    data.tex     = tex;
    data.box     = {std::round(wsBox.x + (wsBox.w - tex->m_size.x) / 2.), std::round(wsBox.y + wsBox.h + margin), tex->m_size.x, tex->m_size.y};
    data.clipBox = clip;
...
void CHyprspaceWidget::draw() {
    ...
    if (g_labelCacheDirty) {
        clearLabelCache();
        g_labelCacheDirty = false;
    }
    ...
    // labels take their height out of the workspace boxes, so the panel keeps its size
    const bool   showLabels     = config.showLabels->value();
    const int    labelFontSize  = std::round(config.labelFontSize->value() * owner->m_scale);
    const double labelMargin    = config.labelMargin->value() * owner->m_scale;
    double       labelSpace     = 0;
    if (showLabels) {
        const auto probe = labelTexture("0", CHyprColor(config.labelColor->value()), labelFontSize, 0, 400);
        if (probe)
            labelSpace = probe->m_size.y + labelMargin;
    }

    // render workspace boxes
    int wsCount = workspaces.size();
    double monitorSizeScaleFactor = (((config.panelHeight->value() - 2 * config.workspaceMargin->value()) * owner->m_scale - labelSpace) / (owner->m_transformedSize.y)); // scale box with panel height
    ...
    double curWorkspaceRectOffsetY = !config.onBottom->value() ? (((config.reservedArea->value() + config.workspaceMargin->value()) * owner->m_scale) - curYOffset->value()) : (owner->m_transformedSize.y - ((config.reservedArea->value() + config.workspaceMargin->value()) * owner->m_scale) - workspaceBoxH - labelSpace + curYOffset->value());
    ...
    *workspaceScrollOffset = std::clamp<double>(workspaceScrollOffset->goal(), -workspaceOverflowSize, workspaceOverflowSize);
    ...
    for (auto wsID : workspaces) {
        ...
        if (showLabels) {
            const bool active = ws && ws == owner->m_activeWorkspace;
            renderLabel(curWorkspaceBox, workspaceLabel(wsID, ws), CHyprColor(active ? config.labelActiveColor->value() : config.labelColor->value()), active ? 700 : 400,
                        labelMargin, labelFontSize, monitorClip);
        }
