...
struct SConfig {
    ...
    SP<Config::Values::CIntValue> showLabels;
    SP<Config::Values::CIntValue> labelFontSize;
    SP<Config::Values::CIntValue> labelMargin;
    SP<Config::Values::CStringValue> labelFont;
    SP<Config::Values::CColorValue> labelColor;
    SP<Config::Values::CColorValue> labelActiveColor;
};
...
inline SConfig config = {
    ...
    .showLabels = makeShared<Config::Values::CIntValue>("plugin:overview:showLabels", "draw \"id: name\" under each workspace", 1),
    .labelFontSize = makeShared<Config::Values::CIntValue>("plugin:overview:labelFontSize", "label font size, in logical pixels", 14),
    .labelMargin = makeShared<Config::Values::CIntValue>("plugin:overview:labelMargin", "gap between a workspace and its label", 6),
    .labelFont = makeShared<Config::Values::CStringValue>("plugin:overview:labelFont", "label font family; empty uses misc:font_family", ""),
    .labelColor = makeShared<Config::Values::CColorValue>("plugin:overview:labelColor", "label color", CHyprColor(1, 1, 1, 0.7).getAsHex()),
    .labelActiveColor = makeShared<Config::Values::CColorValue>("plugin:overview:labelActiveColor", "label color of the active workspace", CHyprColor(1, 1, 1, 1).getAsHex())
};

// Set on config reload; draw() drops the cached label textures when it sees it.
inline bool g_labelCacheDirty = false;

// Frees the cached label textures (Render.cpp).
